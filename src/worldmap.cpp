...
namespace iso {
    ...
    void Worldmap::initialise() {
        this->panel_size = sf::Vector2i(64, 32);
        ...
        this->view_game.setCenter(sf::Vector2f(
            ...
            this->panel_size.y * this->settings.size.y / 2
        ));

        this->view_game.setSize(sf::Vector2f(
            this->panel_size.x * this->settings.size.x / 15,
            this->panel_size.y * this->settings.size.y / 15
        ));
    ...
    void Worldmap::moveCamera() {
        auto distance = sf::Vector2f(
            (this->position_pressed.x - this->position_released.x) / this->panel_size.x / 2,       
            (this->position_pressed.y - this->position_released.y) / this->panel_size.y / 2
        );
        ...
        if(this->view_game.getCenter().x + (distance.x * x_multiplier) < (this->settings.size.x * this->panel_size.x) - this->view_game.getSize().x / 2.f && this->view_game.getCenter().x + (distance.x * x_multiplier) > 0.f + this->view_game.getSize().x / 2.f) {
            this->view_game.move(x_multiplier * distance.x, 0.f);
        }

        if(this->view_game.getCenter().y + (distance.y * y_multiplier) < (this->settings.size.y * this->panel_size.y) - this->view_game.getSize().y / 2.f && this->view_game.getCenter().y + (distance.y * y_multiplier) > 0.f + this->view_game.getSize().y / 2.f) {
    ...
    void Worldmap::updateCamera() {
        ...
        if(this->view_game.getCenter().x + this->view_game.getSize().x / 2 > this->panel_size.x * this->settings.size.x) {
        ...
        if(this->view_game.getCenter().y + this->view_game.getSize().y / 2 > this->panel_size.y * this->settings.size.y) {
    ...
    void Worldmap::highlightPanel() {
        auto grid_position = sf::Vector2i(
            (int)this->mouse_position_window.x / this->panel_size.x,
            (int)this->mouse_position_window.y / this->panel_size.y
        );

        auto position = sf::Vector2f(
            grid_position.x * this->panel_size.x,
            grid_position.y * this->panel_size.y
        );

        sf::VertexArray highlight(sf::Quads, 4);

        highlight[0].position = position;
        highlight[1].position = position + sf::Vector2f(this->panel_size.x, 0);
        highlight[2].position = position + sf::Vector2f(this->panel_size.x, this->panel_size.y);
        highlight[3].position = position + sf::Vector2f(0, this->panel_size.y);

        highlight[0].texCoords = sf::Vector2f(0, 0);
        highlight[1].texCoords = sf::Vector2f(this->panel_size.x, 0);
        highlight[2].texCoords = sf::Vector2f(this->panel_size.x, this->panel_size.y);
        highlight[3].texCoords = sf::Vector2f(0, this->panel_size.y);
    ...
    void Worldmap::drawInterface() {
        ...
        sf::Vector2i grid_position = sf::Vector2i(
            this->mouse_position_window.x / this->panel_size.x,
            this->mouse_position_window.y / this->panel_size.y
        );
