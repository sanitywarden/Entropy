...
void WidgetRegionStorage::updateUI() {
    ...
    //     if(current_region->itemExists(item))
    //         item_total++;

    // for(const auto& item : ITEM_LOOKUP_TABLE) {
    //     if(current_region->itemExists(item)) {
    //         if(!current_region->itemQuantity(item))
    //             continue;
    ...
    //         auto text = LabelComponent(new Label(this->manager, std::to_string(current_region->itemQuantity(item))));
